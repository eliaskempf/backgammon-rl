# ruff: noqa
# Vendored verbatim from dellalibera/gym-backgammon
# (gym_backgammon/envs/backgammon.py). See the upstream repository for license
# and attribution.
#
# TEST-ONLY: used as a differential-testing oracle for bgrl's own move generator
# (bgrl.env.movegen). Do NOT import this from any production code path. Kept
# verbatim and excluded from ruff so it can be re-synced from upstream.
#
# Conventions captured from upstream (needed to drive it as an oracle):
#   * board: list[ (count:int, color:int|None) ] of length 24; WHITE=0, BLACK=1.
#   * bar/off: [white, black].
#   * WHITE moves toward index 0 and is driven with NEGATED rolls, e.g. (-3,-1);
#     BLACK moves toward index 23 with positive rolls, e.g. (3,1).
#   * get_valid_plays(player, roll) returns the set of max-dice plays; each play
#     is a tuple of (src, dst) submoves with BAR="bar" src and off-board dst.
#   * execute_play(player, play) mutates board/bar/off (handles hits/bar/bear-off).
import itertools
